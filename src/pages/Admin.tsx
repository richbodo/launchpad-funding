...
export default function Admin() {
  ...
  const sendWelcomeEmail = async () => {
    ...
    try {
      ...
      // Look up the row id so the sent status persists (the dialog only carries
      // email/name/role).
  ...
  const handleBulkCsv = async (file: File) => {
    ...
    try {
      ...
      // Map each header column to a role.
      ...
      if (!roleForCol.some(Boolean)) {
        toast.error('No recognised columns. Use the template: Investor email, Start Up email, facilitator email.', { duration: 15000 });
        return;
      }

      // Collect unique (email, role) pairs from the data rows.
      const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      ...
      // Skip emails already on the session.
      const existing = new Set(participants.map(p => p.email.toLowerCase()));
      let added = 0;
      let skipped = 0;
      let failed = 0;
      // Track next presentation order for startups across the whole import.
